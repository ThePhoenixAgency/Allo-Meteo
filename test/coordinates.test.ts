...
const LOCATION_COORDS = { lat: 45.0520, lon: 6.0301 };
...
describe('Coordonnées GPS', () => {
  ...
  it('Toutes les stations doivent avoir des coordonnées valides', () => {
    Object.entries(STATIONS_COORDS).forEach(([name, coords]) => {
...
describe('API Prevision-Meteo.ch', () => {
  ...
  it('doit construire l\'URL correctement pour Le Bourg d\'Oisans', async () => {
    ...
    (global.fetch as any).mockResolvedValueOnce({
      ok: true,
      status: 200,
      json: async () => ({ current_condition: { tmp: 5, humidity: 70 } })
    });
  ...
  it('doit gérer les timeouts', async () => {
    ...
    (global.fetch as any).mockRejectedValueOnce(new Error('AbortError: The operation was aborted'));
